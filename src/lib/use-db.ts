...
import dbFactory from './use-template-db';
...
const DbTestUtil = {
  ...
  teardown: async function ({ forceDelete } = { forceDelete: [] }) {
...
const initDb = async ({ cwd, database_extra }: any) => {
  const { configModule } = getConfigFile(`${cwd}/..`, `medusa-config`)
  const { projectConfig, featureFlags } = configModule as any
